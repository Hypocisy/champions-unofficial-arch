...
public final class MouseHelper {
  ...
  public static Optional<LivingEntity> getMouseOverChampion(Minecraft mc, float partialTick) {
    ...
    if (entityHit == null) return Optional.empty();
    Entity hit = entityHit.getEntity();
    if (!(hit instanceof LivingEntity living)) return Optional.empty();
    if (!ChampionsApi.get().isChampion(living)) return Optional.empty();
